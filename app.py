...
# Load models and class mappings (dummy placeholders for now)
models = {
...
def load_image(img_path):
    ...
    img = img.resize((299, 299))  # Resize to model input size
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if img_array is None:
                flash("Invalid image. Please upload an Valid image.", "error")
                return redirect(request.url)
